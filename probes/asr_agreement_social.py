    ...
    with tempfile.TemporaryDirectory() as td:
        for name in wavs:
            ...
            try:
                ...
                ta = norm(re.sub(r"<\|[^|]*\|>", " ", ra[0]["text"] if ra else ""))
                segs, info = b.transcribe(clip, beam_size=1)
                tb = norm(" ".join(s.text for s in segs))
            except Exception as e:                                # noqa: BLE001
                rows.append((name, "失败", str(e)[:50])); continue
            if not ta and not tb:
                rows.append((name, "两侧皆空", "")); continue
            # 对称一致率: 1 − 归一化编辑距离(以较长一侧为分母, 避免偏袒空输出)
            d = wer(ta, tb) if len(ta.split()) >= len(tb.split()) else wer(tb, ta)
            agr = max(0.0, 1.0 - d)
            agrees.append(agr)
            rows.append((name, f"{agr:.3f}", f"A {len(ta.split())} 词 / B {len(tb.split())} 词"))
