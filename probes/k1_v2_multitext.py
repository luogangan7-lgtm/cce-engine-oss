#!/usr/bin/env python3
"""K1-v2 多文本判定 —— 5 文本 × n=8, 同批同时判 intensity 与 weight。

前登记: tests/data/phase2/k1_v2_multitext_prereg.json (跑前已冻结, 含修订记录)

★ 纪律:
  · 判据用 probes/k1_gate.judge(), 一字不改
  · 文本由字典序规则冻结, 不许因结果不好换
  · INFRA_FAILED 不算一次读数, 该 rep 重试, 不进 n
  · **非退化检验**: 高一致率 + 零区分度 = 这个层什么都没测(见预注册 degeneracy_guard)
"""
import json
import os
import statistics
...
CKPT = P / "k1_v2_checkpoint.jsonl"
SPEC = json.loads((P / "k1_v2_multitext_prereg.json").read_text(encoding="utf-8"))
...
def report(TS, inst) -> int:
    ...
    out = P / "k1_v2_multitext_verdict.json"
