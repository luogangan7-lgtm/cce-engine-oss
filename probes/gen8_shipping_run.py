#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""候选代一轮 —— 按 tests/data/gen8_shipping_prereg.json 冻结的计划执行。
...
# ★★★ 上线配置: decision_tree **开**(owner 裁定)。与 gen7 那轮**不是同一个配置**。
COMPONENTS = ("ontology", "discriminant", "negative", "full_behavior", "decision_tree")

OUT = ROOT / "results" / "gen8_shipping"
PREREG = json.loads((ROOT / "tests/data/gen8_shipping_prereg.json").read_text(encoding="utf-8"))
ASSERTS = json.loads((ROOT / "tests/data/local_contract_assertions_v2.json").read_text(encoding="utf-8"))
...
def main():
    ...
    (OUT / "run_params.json").write_text(json.dumps({
        ...
        "★上线组件": list(COMPONENTS),
        "★decision_tree": "**开** —— owner 裁定; 独立性代价见 cce_stage2_candidate.INDEPENDENCE_COST, 换代后必须同步声明 G 的判决口径变化",
        "上线 s2 prompt 字数": len(V.build(taxo, "SAMPLE", {"tops": {}, "appraisal": {}}, COMPONENTS)),
    ...
    for a in adjudicated:
        ...
        if not r or r.get("top1") is None:
            checked.append({"用例": a["用例"], "谓词": a["输出谓词"],
                            "结果": "NO_READING(未产出读数, 不计通过也不计违例)"})
            continue
        forbidden = _re.search(r"top1 != '(\w+)'", a["输出谓词"]).group(1)
        ok = r["top1"] != forbidden
        checked.append({"用例": a["用例"], "top1": r["top1"], "谓词": a["输出谓词"], "符合": ok})
        if not ok:
            violations.append({"用例": a["用例"], "top1": r["top1"], "断言": a["输出谓词"],
                               "条款": a["条款定位"], "失败的合取项": a["失败的合取项"]})

    verdict = {
        "★★★被测单位数": len(rows),
        "已裁定断言数": len(adjudicated),
        "未裁定用例(只记录, 不进分母)": len(ASSERTS["未裁定用例"]),
        "★★★已确认违例": violations,
        ...
        "★上限": REQUEST_CAP,
        "未裁定题的读数(诊断用, 无判据)": [
            {"用例": i, "top1": by_id.get(i, {}).get("top1")} for i in ASSERTS["未裁定用例"]],
    }
