#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""抽取器构造反例 **r2** —— 判据见 tests/data/extractor_counterexample_prereg_r2.json(测量前冻结)。
...
PREREG = json.loads((ROOT / "tests/data/extractor_counterexample_prereg_r2.json").read_text(encoding="utf-8"))
TPL = json.loads((ROOT / "tests/data/extractor_counterexample_templates_r2.json").read_text(encoding="utf-8"))
CAP = PREREG["执行"]["★请求硬上限"]
# ★ 输出路径可由环境变量覆盖 —— **重复测量必须用同一份代码**。
#   2026-09-14: 分叉出一份 r3 执行器的做法已被否决(旧闸只守旧文件, 配对变异实测 96 道全绿),
#   所以重复测量走**同一个文件**, 只换落盘位置。除路径外**一字未动**。
OUT = pathlib.Path(os.environ.get("CCE_EXTRACTOR_OUT")
                   or (ROOT / "results" / "extractor_counterexample_r2.json"))
...
def summarize(rows):
    ...
    verdict = ("COUNTEREXAMPLE_FOUND" if n_up else
               "DEGENERATE_ON_CONTROLS" if not p_up else
               "NO_COUNTEREXAMPLE_IN_THIS_SUITE")
    ...
    return {"逐臂": per, "阴性合计": neg, "正证据对照": pos,
...
def main():
    ...
    S = summarize(rows)
    res = {"block": "EXTRACTOR_COUNTEREXAMPLE_RESULT_R2",
           "prereg": "tests/data/extractor_counterexample_prereg_r2.json",
           "templates": "tests/data/extractor_counterexample_templates_r2.json",
           ...
           "★★★逐臂": S["逐臂"], "阴性合计": S["阴性合计"], "正证据对照": S["正证据对照"],
           # ★★★ 下面这几段**必须随产物走**。r1 的产物带过「怎么读这个判决」, r2 初版丢了 ——
           #   而半年后只读结果文件、不读预注册的人, 拿到的就是这份文件。
           "★★★承重臂是哪一臂": "**缺P阴性**。那一臂的共享对象名确实存在, 通不过只能靠 P 的五类判断; "
               "对象错配臂可能被逐字要求暗示(已知局限①), 它的干净通过**不算强证据**。",
           "★★★不得据此说": PREREG["★★★不得据此说"],
    ...
    for a_ in ARMS:
        x = S["逐臂"][a_]
        print("  %-10s n=%d 交卷=%s 拒答=%s 裁决过=%s p_challenge=%s"
              % (a_, x["n"], x["交卷"], x["拒答"], x["★被资格层裁决过"],
                 x["★p_challenge(分母=被裁决过的)"]))
    po = S["正证据对照"]
