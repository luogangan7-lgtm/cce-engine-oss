...
### §19.5.53 [MEASURED 2026-09-04] 源分离判出了我前一天上线的韵律模块有问题

**预注册先于测量入库**（commit `c0bdc32`），判据：ρ(非人声能量占比, |Δf0| 半音) ≥ 0.4 且 p < 0.05。

n=30 份真实 WAV。非人声占比跨 **0.002–0.996**（sd 0.309，非退化）。
**Spearman ρ = 0.618, p = 0.0003 ⇒ SEPARATION_MATTERS**。|Δf0| 中位 **12.4 半音 = 一个八度**。

| 非人声占比 | 混音 f0 | 人声轨 f0 | Δ |
|---|---|---|---|
| 0.008 | 244.8 Hz | 244.1 Hz | 0.05 半音 |
| 0.633 | 68.4 Hz | 204.5 Hz | 18.96 半音 |
| 0.996 | 79.5 Hz | 231.4 Hz | 18.50 半音 |

高 BGM 时混音 f0 落在 68–79 Hz —— 那是**贝斯线**，不是人说话。

⇒ 韵律读数现在带 `f0_source`(vocals/mixed) + `nonvocal_share`，由 `prosody_usable()` 判可用：
**说不清算在哪条轨上的一律不可用**；混音且非人声占比 > 0.35 不可用；占比**未知**也不可用 —— 未知不等于低。

★ 只说「混音那条不可信」，**不说「分离后更准」** —— 没有 f0 地面真值就没有准不准。
★ 也不用「分离后方差更小 = 更好」：方差小可能只是把有声段削成了静音。**更稳不等于更对。**

### §19.5.54 [MEASURED 2026-09-04] playbook 原子分解：改善是真的，但差一个也是差

playbook_hit 判 UNRELIABLE(4/8)。失败的形状很具体：4 个 PASS 里 3 个中位数 0.0、1 个 1.0 —— **只在地板和天花板稳**；4 个 FAIL 全在中段。
而读数形态实测（2026-09-03）：cardinal 0.712 / band3 0.726 / rank 0.856 / top2 0.814 / **top1 1.000**。

⇒ 可证伪的假设 H1：**不稳定来自 0–1 标尺本身**。改法：playbook 原文本就是分号分隔的离散动作，逐条问「做了没有 + 逐字引用」，复合值由**算术**得出而非由判官在标尺上取点。

**同一批 8 文本 · 同一仪器(565470cf26c16d01) · δ 与判决线逐字同 GEN4。**
结果 **6/8 INCONCLUSIVE**（需 7）。两条非退化闸都过。⇒ **不采纳**（与 playbook_mode 同处理）。

★ 更锐的发现：**残余不稳定不在标尺上。** `belong` 的 2 个原子里一条是禁令，正向只剩 **1 条**，复合值退化成单个二值判断 —— 而它在 0/1 之间来回摆（一致率 0.571，极差 1.00）。**连二值都不稳，拆得再细也没用。**
⇒ H1 只被**部分**支持；那部分 H0 未被推翻。再推一版要动的是 **playbook 原子本身**（措辞太抽象，无法逐字指认）—— 那是改**干预设计**（效度）不是改测量（复现性），**需 owner 拍板**且要另立预注册。

### §19.5.55 [MEASURED 2026-09-04] OCR 抽取质量：上界曲线 + 真实素材 n=6，两者不是一回事

**上界曲线**（合成图，零调用）：中英各 6 条，四条退化轴各自单独扫。
- 模糊 σ：英 3.0 仍满分、4.5 塌到 0 | 中 3.0 已掉到 0.90
- 缩放：英 0.2 还有 0.93 | 中 0.2 就塌到 0（**中文更早崩**）
- JPEG 与对比度：测试范围内**无分辨力**

**真实素材**（抖音封面 6 张，人工逐字标注）：中位 **0.900**，范围 **0.25–1.0**，零文字对照上**无中生有 0 次**。
最低两例正是难例：四字散在 2×2 拼图四格 **0.25**；标题两端被裁 + 半透明水印 **0.545**。

★ 两者互证：合成无退化 1.000 ≥ 真实中位 0.900 ⇒ **「上界」这个框架成立**。
★ **英文域仍无一张标注素材 ⇒ 那一半仍未测。** 该能力是语言相关的，中文的数**不许外推到英文**。

#### ★ 第一版跑出「四条轴全无分辨力」，差一点被读成「OCR 很稳健」

实为**仪器坏了**：PingFang 加载失败回退 Helvetica ⇒ 中文渲成豆腐块，全 0.00；退化档对 44px 干净黑字也太轻。**那条曲线量的是我的渲染器，不是 OCR。**

⇒ 两条闸补上：
1. **渲染器自检** —— 无退化时读不回原文直接红，不出曲线
2. **物理见证** —— 每一档记 RMS 对比度 / 字节数 / 像素尺寸。「这条轴没分辨力」在「仪器坏了」和「OCR 真稳健」之间**必须可分**。实测见证：对比度 RMS 43.5→2.6、JPEG 9383→2464 字节 ⇒ 退化确实施加了 ⇒ 是真稳健。

### §19.5.56 图片档晋升 —— 记的晋升条件本身自相矛盾

注册表记的是「CI 全链回放 **+ 真实素材**」。去做才发现两半**不能同时满足**：

真实素材带真名，而边界闸 `check_boundary.py` 是**文本闸** —— 它 `read_text(errors="ignore")` 逐文件扫身份字面量。**PNG 像素里的人脸、截图上渲染的 handle，它结构上看不见**，会报**假绿**。我差点据「边界闸 PASS」把真实素材提交进公开仓。

⇒ 拆成两半，各自可验，缺一不算：

| | 做法 | 结果 |
|---|---|---|
| ① 真实素材 | 本机回放 `probes/image_chain_real_material.py` | **6/6 PASS** |
| ② CI 全链 | 现场合成确定性图片，run `33840200869` | 全绿，CI 上**真跑了 OCR** |

配套闸 `tests/test_cce_gate_no_binary_media.py`：公开仓**结构上**不放媒体二进制，补住边界闸对像素的盲区。反向自检：埋一个真 PNG 必须被判出。

### §19.5.57 同一类错今天犯了六次 —— 靠记性无效，靠闸

| # | 形状 |
|---|---|
| ① | `test_cce_strategy_gate` 断言「可发布」，而 `check_boundary` 在仓外保险库 |
| ② | `test_cce_no_real_identities` 运行时从保险库读化名表 |
| ③ | `test_cce_audio_prosody` 无 wav 时回退 `__file__`，CI 上把 .py 喂给 librosa |
| ④ | `test_cce_media_ingest_production` 有存在性判断，但**缺席就静默跳过** |
| ⑤ | `test_cce_visual_prose_gap` 自己没有仓外路径，但 **import 的探针**要本机素材 |
| ⑥ | 同一文件：我加了 `if r is not None:`，本机绿 CI 照样红 —— 无素材时它返回的是**结构完整但字段为 None 的 dict**。**我读代码推断了缺席形态，没去跑。** |

⇒ `tests/test_cce_no_offrepo_dependency.py`：仓外绝对路径必须**同时**有存在性判断 + **明写缺席时的行为**；并扩到**间接依赖**（import 了「自身引用仓外路径的探针」的测试）。判据收窄两次以避免误报 —— **误报会训练人忽略这条闸**。
⇒ 习惯：**降级路径要实际强制触发一次**（把 SRC 指到空目录跑一遍），不许靠读代码断言。

★ 共同点：**「优雅降级」不等于「静默跳过」。** 降级路径必须自己也有断言，否则就是恒绿。

### §19.5.58 我自己刚写的一个闸违反了全占比原则 —— 而且它还不可达

韵律可用性判据我第一版写成 `nonvocal_share > 0.35 ⇒ 不可用`。两条理由删掉它：

1. **全占比原则明令禁止「在噪声连续量上设布尔闸」。** 0.34 与 0.36 在测量上不可区分，判决却相反。而 0.35 是我拍的数，不是从数据推的。
2. **它不可达。** `nonvocal_share` 只在分离成功那一支被赋值，而那一支 `f0_source` 就是 `vocals` ⇒「mixed 且占比已知」这个组合在代码里根本不存在，那条分支是死代码。

⇒ 判据改成纯**类别**的：算在人声轨上 = 可用；算在混音上 = 不可用（此时占比必然未知）；其他/缺失 = 默认拒判。**类别判断不受连续量噪声影响，也不需要我拍阈值。**

`tests/test_cce_prosody_usable.py` 钉住三件事：判据是类别的、阈值不存在（占比 0.0–0.99 都不改变判决，证明没有阈值在暗处）、删掉阈值的**理由**留在原地（否则下一个人会把它加回来）。

★ 若将来真出现「混音但占比已知」的路径，阈值必须**从数据重新推**，不许继承这个被删掉的猜测。

### §19.5.59 [OBSERVED 2026-09-04] 重验三档发现：同一份输入，可用读数集会变

今天动过 workflow YAML，所以把 09-03 验过的三档在**当前代码**上重跑一遍。三档全绿（`outbound_post` 33842081104 · `outbound_reply` 33842084279 · `subject_chain` 33842088048），但**可用读数集变了**。

信封与旧 run **逐位相同**（只改 `submission_id`/`trace_id`，已用脚本核对）；两处闸的代码与常量最后改动是 **2026-08-18**，早于两次 run。⇒ 输入与判据都不变。

| profile | s1.tops.need | s2.playbook_primary |
|---|---|---|
| outbound_post | 可用 → **扣发**（within_js=0.3158 > 0.161） | 可用 → **扣发**（top1 不稳：belong/pain_seek/pain_seek/pain_seek/belong） |
| outbound_reply | 可用 → **扣发** | **扣发 → 可用**（方向相反） |

★ `s2.playbook_primary` 在两档里**方向相反** —— 这条排除了「今天的代码把闸改严了」这个解释。是估计量的随机性。

★ `s1.tops.need` 在**两档都**由可用转扣发。方向一致，但 **n=2** ⇒ **登记为待测，不下结论**。要判它需要预注册 + 同输入多次重跑。

#### 结论：「可用」不是常态承诺

标「可用」的读数，其可用性是**逐次运行由闸在运行时判**的，**不是系统的固有属性**。下游**不得**假定某个读数下次还在。这与已测的 θ 敏感性（18.4% 判决被 weight 抖动翻转）是同一族问题。

★ 这**不**是说读数没用 —— 闸正常工作，不稳时如实扣发，这正是设计意图。只是不能把某一次的可用集当成常态承诺。

★ n=2 且**无预注册** ⇒ 这是**观察**，不是翻转率。`tests/test_cce_readout_usability_flip.py` 由**归档产物**复核而非信我写的摘要。

#### 顺带：为什么要重跑而不是读代码

今天改的只有 workflow YAML，而且改的是一个 `if: profile == 'media_ingest'` 的条件步骤 —— 按代码读，它对出站档无影响。**但我今天已经两次栽在「读代码推断而不去跑」上**（§19.5.57 的 ⑤⑥）。跑了才拿到上面这张表；不跑就什么也不会发现。

### §19.5.60 [MEASURED 2026-09-04] 两条「结构性做不了」是我分错了类

owner 让我「用网页 GPT 调研解决」，翻出两条我报错的判断。

#### ① 抽取质量：域相关 vs 语言相关

我登记「需**英文域的带标注素材** ⇒ BLOCKED_EXTERNAL」，并据此向 owner 报「结构性做不了」。
**错**：我把「需要标注素材」读成「需要**本域**（助听器）的标注素材」。而本项目自己的分解是 **能力＝域无关 / 抽取质量＝语言相关 / 标定＝域相关** —— 抽取质量既然只与语言有关，**公开英文基准就是正确的素材**。

⇒ 这是**分类错误，不是资源短缺**。

#### ② 说话人分离：包 vs 权重

我登记「pyannote 是受限模型，需 HF token ⇒ 拿不到凭据」。**两处错**：
1. `pyannote.audio` 是 **MIT 开源包**，受限的是**权重** —— 装包无需 token
2. 3D-Speaker 的默认路径 `include_overlap=False` **根本不碰** pyannote 权重（源码：`if include_overlap and hf_access_token is None: raise`）

⇒ 无账号、无 token、无点击条款、无许可限制。实测 CPU **0.133x 实时**。

### §19.5.61 [MEASURED 2026-09-04] 英文抽取质量与 DER

| 读数 | 结果 |
|---|---|
| OCR 合成干净渲染 | **1.000**（上界） |
| OCR 真实中文封面 n=6 | **0.900** |
| OCR 英文密集场景 TextOCR n=98 | **0.317**（顺序无关词级 F1，下界） |
| ASR test-clean 40 人 | 均值 WER **2.34%** |
| ASR test-other 33 人 | 均值 WER **11.61%**（**4.96×**） |
| DER STRICT（collar=0，计重叠） | **0.1004** |
| DER LEGACY（collar=0.25，不计重叠） | **0.0338** |
| CPU RTF（本机铸值） | **0.133** |

**报区间不报点估计。** 且 test-clean 与 test-other **分别报，不合并** —— 难度一变结论就变，合并等于用一个数掩盖带噪表现。

#### ★ 三条「先验仪器再报数」

1. **顺序无关的词级 F1**：序列编辑距离对**我的拼接顺序**极敏感，只报它会把我的拼接方式当成 OCR 的错。实测 F1（0.317）反而**低于**序列准确率（0.554）⇒ 低分是真的没读出来。
2. **独立第二引擎**：预注册判据「与公开报告值比对」在 RapidOCR×TextOCR 上**无公开值、不可执行** —— 如实记下，**追加**（非替换）等价自检：同 100 张图 tesseract 词级 F1 中位 **0.0** ⇒ 我的集成没坏，是基准难。
3. **ASR 最差 6 条逐条查看**：全是真实识别错误（专有名词 / 英式拼写 / 连读），**不是**数字缩写格式假象。

#### ★ 抽样规则执行对了，后果仍可能坏

GEN1 严格按预注册的「字典序前 100」抽 —— 结果只覆盖 **2 个说话人**（test-clean 共 40）。
**不改 GEN1 的规则**（看到结果后改预注册是明令禁止的），另立 GEN2 分层抽样。两个都报。GEN2 中位落在 GEN1 四分位内 ⇒ `GEN1_CONSISTENT`。

#### ★ 四个准入字段永不合并

`config/cce_eval_benchmarks_v1.json` 逐库记 account_required / token_required / click_through_terms / license_restriction。反例：

- sherpa-onnx reverb 权重：可匿名直链下载（前三项全 false），但许可 **non-commercial**
- NeMo/NGC：可匿名 curl，但**下载即视为接受条款**
- CORD：许可与可得性都好，但它是**印尼**收据 ⇒ 语种不对，差点被我选中
- SpeechBrain：权重开放，但**没有 turnkey diarizer** ⇒ 自己拼会把大量测量自由度带进 benchmark。**开放不等于适合当验收实现。**

#### ★ DER 低 ≠ 说话人数对

说话人**数**预测正确 **14/20**。两者是不同读数 —— 下游若要用「有几个人说话」，用的是后者，**不许拿 DER 给它背书**。

### §19.5.62 [MEASURED 2026-09-04] outbound_post 的结层读数恢复 —— 「未测」归零

生产状态表里唯一的「未测」是：**outbound_post 用的仪器 `0e9ca1d4e7a2f180` 上没有 K1 判定**。K1-v2 是在 `565470cf26c16d01`（k=3）上做的，而**标定不可跨仪器搬** ⇒ 该档结层零可用读数。这不是「弱证据」，是没有读数。

⇒ 在**那台仪器上**补做 K1（`K1_V2_K5_INSTRUMENT`，预注册先于测量）：

- 已反解命中该仪器 = **k=5 + `s1_pairing='round_robin_over_5_s1_draws'`**，并与归档 run `33842081104` 产物里记的 instrument 字段一致
- 判据、文本与 K1-v2（k=3）**逐字相同**，只有仪器不同 ⇒ 差异只能归因于仪器
- k=3 的 raw draw **一条都不复用**，40 次调用

| 层 | k=3 (565470cf) | k=5 (0e9ca1d4) |
|---|---|---|
| intensity | 0/5 | **0/5** |
| weight | 0/5 | **0/5** |
| top-1 | 8/8 | **五文本全 8/8** |

⇒ `INSTRUMENT_WIDE_FAIL` 在两台仪器上**同形** —— 失败不是 k=3 特有的。而 **top-1 在两台上都满分**，非退化（3 种众数：pain_seek / reward / belong）。

#### ★ 加第二台仪器**不是**放松「不可跨仪器搬」

`cce_k1_status` 由「单一判定文件」改为 `VERDICT_BY_INSTRUMENT` 路由 —— **每台仪器必须有它自己的判定，未登记的仪器与缺仪器标识一律拒发**，且两种拒发理由**分开**（「压根没给标识」与「给了但那台没判定」是两回事，合并会让日志读不出问题在哪）。反向测试验了 4 种情形。

#### ★ 一个差点被我悄悄弄坏的反向测试

第一版把判定路径在**导入时**固定进字典。而 `test_cce_measurement_system` 靠 monkeypatch `K1_VERDICT` 来证明「闸是判定驱动的，不是恒拦」—— 路径一旦在导入时固定，那条反向测试就失效了，**而闸看着还是绿的**。

⇒ 字典里存的是**模块全局变量名**，路径在**调用时**解析；并加了一条断言：把 `K1_VERDICT` 改指到不存在的文件，闸必须变红。

**恒拦的闸和恒放的闸一样没有信息量。**

### §19.5.63 [MEASURED 2026-09-04] 社媒音轨上的 ASR：拿到可证的上界

此前只能靠推测说「朗读语音的数对本项目素材仍是乐观值」。现在有了可证的约束。

**做法**：我听不了音频，做不了逐字标注；公开集里也没有本项目域的带标注社媒音轨 ⇒ **真准确率测不了**。能测的是两个**相互独立**的引擎在同一批真实音轨上的一致性。

- A：`iic/SenseVoiceSmall`（生产用）
- B：`faster-whisper small`（Apache-2.0，独立实现与训练数据）

**结果**：18 份真实社媒音轨（全 CJK），跨引擎一致性中位 **0.475**，范围 0.000–0.818，非退化。

#### 可证的上界（不是手挥）

设 a、b 是两个转写与（未知）真值的匹配率，p 是两者**彼此**的一致率。两者**同时**匹配真值的位置必然也彼此一致 ⇒ 同时匹配 ≤ p。由容斥 a + b − 1 ≤ p：

> **a + b ≤ 1 + p**

实测 p = 0.475 ⇒ a + b ≤ 1.475 ⇒ **至少一个引擎的匹配率 ≤ 0.738**；若两者水平相当，则**各自都 ≤ 0.738**。

对照：同一个 SenseVoice 在 LibriSpeech test-clean 上匹配率 ~0.977。
⇒ **朗读语音的数确实高估了本项目素材上的表现。**

★ 这**不能**推出生产用的那个就是差的那个 —— 但也**不能**证明它是好的那个。
★ 一致 ≠ 准确：同类模型可能犯同样的错（共同模式失败）。真准确率**仍未测**。

#### ★ 第一版是坏的，靠非退化闸拦下

我直接复用了 `asr_quality_en.norm()` —— 那是 **LibriSpeech 的英文**归一化，只保留 `[A-Z0-9' ]`，**中文被整个抹掉** ⇒ 一致性全 0.000、零方差。

**那是仪器坏了，不是结果。**

⇒ 归一化改为**语种感知**（CJK 按字符 / 拉丁按词），加非退化闸，并加**自比基准**（同一输入自己跟自己比必须满分 —— 归一化一坏这条先红）。

### §19.5.64 「全同值 + 零方差」是仪器故障的签名 —— 一天拦下四次

| # | 现象 | 实为 |
|---|---|---|
| ① | OCR 上界曲线四条轴**全无分辨力** | 中文用 PingFang 加载失败回退 Helvetica，渲成豆腐块 |
| ② | 视觉散文 Jaccard≈0.008 | 中文分词 `[一-鿿]{2,}` 抓出 8–11 字整句 |
| ③ | 英文 OCR 序列准确率偏低 | 序列编辑距离对**我的拼接顺序**极敏感（加顺序无关 F1 才分得清） |
| ④ | ASR 跨引擎一致性**全 0.000 零方差** | 复用英文归一化，中文被抹掉 |

**共同签名：取值全同 + 零方差。** 真实测量几乎不会这样；出现它，先怀疑仪器，不要怀疑世界。

落成两条做法：**非退化闸**（取值种类过少 ⇒ 判 DEGENERATE/NO_RESOLUTION，**不是**「通过」也**不是**「稳健」）+ **物理见证**（记下证明处理确实施加了的量 —— 没有它，「无分辨力」在「仪器坏了」和「真稳健」之间分不开）。

### §19.5.65 [MEASURED 2026-09-04] 证据基础里的静默 ASR 失败 —— 非退化闸撞出来的

**不是我去找它。** 字幕交叉核验的非退化闸触发（召回中位 0.0098，落在预注册的 (0.05, 0.95) 之外），规则要求「先查仪器」。查下去发现：**参照方没坏，被参照的 ASR 才是坏的**。

#### 实测（预注册 n=40）

| | 短转写（字/秒 < 0.15） | 长转写（≥200 字） |
|---|---|---|
| 人声能量占比中位 | **0.178** | **0.969** |

两组**可分**（差 0.79）。

- **22.5%** 的短转写人声占比 ≥ 0.5（确有口播却近乎空白）= **失败率下界**
- 其中 **9/40 标着 `speech: true`** —— 那是**假的 ok**
- 全集外推 ≈ **31 份**被标完备却近乎空白

★ 判决 `MIXED` 的意思是**两种成因都有**，不许只报一种：77.5% 确实是素材本就无口播，空转写是**正确的**。

★ 这要紧，因为 `media_ingest` 链正是从这 276 份产物跑出 **17,726 条 observation** 的。

#### ★ 一个自己打脸的细节

最初 **6 份抽查**的短转写组人声中位是 **0.472**，看着像「一半全坏」。预注册的 **40 份**给出 **0.178**。

**抽查用来发现问题，不能用来估比例。** 幸好判据要求扩样。

#### 修法：speech 由两态拆成五态

原 `speech = audio.present` 只表示「音轨在不在」，把两件事压成一个 `true`：

| 状态 | 含义 |
|---|---|
| `present` | 转写非空（按**字/秒**判） |
| `absent_verified` | 转写空**且**人声占比低 ⇒ 素材本就无口播，空是**对的** |
| `missing_parse_failed` | 转写空**而**人声占比高 ⇒ **ASR 失败** |
| `not_available` | 转写空，但**查不了**人声占比 ⇒ 不知道 |
| `missing_no_capability` | 无音轨 |

★ 界按**字/秒**而非绝对字数 —— 3 秒 3 字是正常的，178 秒 1 字不是。出处标 **ENGINEERING_BUDGET**（工程预算，不是标定过的阈值），原始量（chars / chars_per_sec / vocals_share）随状态带出，下游可自行重判。

★ **「查不了」不许写成「查过没有」** —— 这条是四态纪律的核心，测试反向验过。

### §19.5.66 ★ 更正 §19.5.65 的规模 —— 我自己的坏闸把发现放大了 15 倍

§19.5.65 报「全集外推 ≈ **31 份**静默 ASR 失败」。**那个数是错的。**

它用**绝对字数**（< 20 字）界定「转写近乎空白」。而 138 份里有 **108 份是短视频里的正常转写**（时长中位 **14.6 秒**）—— 一个 14 秒 15 字的视频 = 1.07 字/秒，完全正常，却被绝对阈值算成「空」。
...
|---|---|---|
| 短转写产物 | 138 | **31** |
| 其中高人声（≥0.5） | 22.5%（n=40 抽样） | **2** |

⇒ 静默 ASR 失败**存在但罕见（约 2 份）**，不是 31 份。**我放大了约 15 倍。**

★ 那个 22.5% 测的是**我自己的坏闸**，不是 ASR 失败率 —— 样本里的高人声条目多半是短视频里的**正常**转写。

#### 仍然成立的

`speech = audio.present` 把「素材本就无口播（空转写是**对的**）」与「ASR 失败（空转写是**错的**）」压成一个 `true` —— **这个缺陷是真的，与规模无关**，五态修法照旧。确实存在高人声却近乎空白的条目（2 份），静默失败**存在**，只是罕见。

#### ★ 第五次仪器故障，签名和前四次相反

前四次的签名是**取值全同 + 零方差**（把结果**归零**）。
这次相反：**坏闸把发现放大了 15 倍**。

⇒ **仪器故障不只会让结果消失，也会让它膨胀。**
新的检查动作：**报一个「有多严重」的数之前，先问界定它的那个闸本身对不对。**

#### 回填的结论

回填已跑：对照组（历史长转写）重跑后 **10/12** 仍达标 ⇒ 我的重跑环境没问题（退化闸通过）。但高人声样本 **n=2 < 8** ⇒ 判 **INSUFFICIENT**，无法回答「是那一轮坏了还是模型对这类素材本就不行」。要定论需更多高人声空转写样本 —— 现有素材里就这么多，属于**素材限制**。

### §19.5.67 [MEASURED 2026-09-04] ASR 受控退化曲线 —— 真实素材恰好落在曲线开始塌的那一点

社媒音轨的 ASR 真准确率测不了（我听不了音频，公开集无本域标注）。调研指出**唯一能拿到真 ground truth** 的路是：**TTS 合成已知文本 + 受控退化**。参考文本是我自撰的 12 句，逐字已知。

12 句 × 7 音色 = **84 条**（合成 84/84，跳过 0）。无退化 CER 中位 **0.0000**。

| 噪声 SNR | ∞ | 10 dB | 5 dB | **0 dB** | −5 dB |
|---|---|---|---|---|---|
| CER 中位 | 0.00 | 0.00 | 0.00 | **0.13** | 0.20 |

| MP3 码率 | 无压缩 | 64k | 32k | 16k |
|---|---|---|---|---|
| CER 中位 | 0.00 | 0.00 | 0.00 | **0.00** |

#### ★ 真实素材在曲线上的位置

真实社媒音轨的**非人声能量占比中位 0.491**（源分离实测）⇒ 等效 **SNR ≈ 0.2 dB**。
而曲线恰好在 **SNR = 0 dB 处从 0.00 跳到 0.13**（10 dB / 5 dB 仍是 0.00）。

⇒ **本项目素材恰好落在曲线开始塌的那一点上。**

★ 0.13 是**下界**，三重理由：① 我用的是高斯噪声，真实 BGM 是结构化音乐 ② TTS 语音比真人干净（无口音/情绪/口误/远近场）③ 真实素材还有混响与多说话人。

#### ★ 一条可直接行动的工程结论

**MP3 降到 16 kbps，CER 仍是 0.00** —— 且见证证明压缩确实施加了（中间 mp3 字节 7425–29133，3.9 倍差）。

⇒ **压缩不是问题，噪声才是。为 ASR 保留高码率音频没有收益；该花力气的是降噪 / 源分离。**

#### ★ 三次仪器故障，三次被闸拦下

1. 第一版写死 `["Tingting","Eddy","Flo"]` —— **Eddy/Flo 是同名的英文音色**，中文输出为空 ⇒ 无退化 CER = 1.0000。改为程序化枚举 + **读回自检**。
2. 本机 `say` 的 9 个 zh_CN 音色只有 **1 个**通过自检，而设计要求音色是有变异的 facet（≥2）⇒ 改用 edge-tts（8 个中文神经音色，7 个通过）。
3. ★ **MP3 轴的见证测错了对象** —— 我在 MP3 往返后又转回 WAV，记的是 WAV 字节（112172 vs 112206，几乎无差），**根本没证明压缩施加了**。改为记**中间 mp3** 的字节。

★ 第 ③ 条尤其要记：**见证本身也会指错对象**，那时「无分辨力」依然与「仪器坏了」分不开。

### §19.5.68 [RESEARCH 2026-09-04] playbook 诊断：调研纠正了我三处，其中一处是我正要踩的坑

#### 诊断（P0×2 / P1×1 / P2×2）

★ 先更正我自己：我提交过「残余不稳定不在标尺上 —— belong 正向原子只剩 1 条」，暗示是**数量**问题。**数据推翻了它**：`pain_seek` 同样只有 1 条正向原子，4/4 全 PASS。

真正的根因是**判定单元与判官能看到的东西不匹配**：

- **P0 不可见原子**：`audit` 的「第二次提及=放大事件」是**跨轮次**规则，单文本判官**结构上看不到** ⇒ 该条 FAIL，中位恰好 0.50
- **P0 一个原子塞多件事**：`belong` 的「先『你不是一个人』把人接住，再给至多一格新知」= 两个动作 + 一个顺序约束 = **三件事压成一个二值** ⇒ 一致率 0.571，**极差 1.00**
- P1 六条原子过短且抽象（2–8 字），其文本大多落在**地板**，一致率虚高
- P2 `display` 无禁令（与其余八结不同构）· `suspend` 的禁令含句号（分号切不开）

#### 系统性：拆原子改变了数字，没改变结构

原子分解的 PASS **6/6 全在地板/天花板**；原始 playbook_hit 的 PASS **4/4** 同样如此。

#### 调研纠正我三处

**① 命名错了。** 不能叫「端点偏好」—— 那意味着 judge *偏爱输出* 0/1；我的数据是**稳定样本恰好落在** 0/1。这是**条件稳定性不同**。正确：`boundary-region instability`。★ 且**没有论文证明**这是普遍定律 ⇒ 只能写成本仪器的实测性质。

**② 机制比我想的更准。** 关键变量是**可核验性**不是数值位置（ACL 2026：可核验属性 accuracy **0.71** vs 主观属性 **0.19**，且 verifiability 比模型选择更能预测 robustness）。中段之所以不稳，是它恰好对应**证据不完整、多解并存 ⇒ 高判定熵**。⇒ 我第二轮往可验证事实移动**不是失败**，是吃到了主要结构性收益（CheckEval 报 agreement **+0.45**，同向）。

**③ ★★ 我正要踩的坑。** 我打算说「地板处一致率 1.000 ⇒ 那是可靠读数」。**那测的是 reliability 不是 validity** —— 有 human-grounded audit 发现 LLM judges 可以彼此高度一致却与人类目标变量不一致。⇒ 要把「稳定区」合法化，**必须有独立的人工 gold 校准集**。

#### 新的测量契约

正式**放弃** `execution_intensity ∈ [0,1]`，改为
`atomic evidence → deterministic composition → {CLEAR_ABSENCE, CLEAR_EXECUTION, ABSTAIN}`
方法学名称：**selective classification**（Chow 1970；Geifman & El-Yaniv 2017）。

★ **不得**定义成「score < 0.2 ⇒ 未执行」—— 那又偷偷依赖了不可靠的连续 score。
必报：coverage · selective risk/accuracy · CLEAR_ABSENCE precision · absence recall · risk–coverage curve。

★ 别再试的（有实证）：pairwise（**改变 estimand**）· 更细 Likert rubric（应变成 executable predicates）· CoT 先理由后分（有研究发现**压塌 judgment distribution 反而更差**）· 多数票（估的是 judge 的 **modal response 不是真值**）· 同义改写 prompt（**本身就会改变结果**）。
