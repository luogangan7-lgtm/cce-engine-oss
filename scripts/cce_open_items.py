    ...
    try:
        ...
        out.append({"类": OPEN,
                    ...
                    "证据": ("原问题: " + rv["⑥★★★新增的 owner 待决项"]["问题"] +
                            ...
                            "② **证据片段的产出方已定**(同日代定): = s2 模型的 evidence_quote(本来就在产出里, 此前从未核过是否逐字在原文); 但 s2 协议不给「支撑哪一支 + 关于哪个对象」⇒ 资格层**结构上只能给候选**, 升格要走 r2 式引用证书协议(另立预注册)。★ 引用证书协议进生产: 2026-09-24 预注册并**试点已跑**(88 次): 判决 **STOP** —— MALFORMED 率 24%>10%, 可判率 24/41(CP95 0.42–0.74), 升格 4/41, 代价中位 20s ⇒ 不接线, 先修采集协议(格式稳定性); ★ 同日已做: 执行器加原因码(无文本) + 采集协议 v3(kind 菜单机械导出·逐字明示·一次修复重问, 验证器不动) + **v3 试点已跑**(96/176 次): 判决 **ADOPT_SHADOW** —— M1 29/41=71% CP[0.54,0.84] · M4 3/41=7% · M3 中位 25s · M2 5/41; 首答格式码 8/84(span 6·JSON 2), 修复救回 2。★ **已接线**(owner 2026-09-24「接吧」): scripts/cce_citation_certificate.py + s2 内开关 CCE_CITATION_CERT —— **线上已开**(owner 2026-09-24, cce-submit.yml 出站两档 + subject_chain 步骤, 同一仓库变量 CCE_CITATION_CERT=0 全关), 接线闸五条 + 变异 6/6。★ **线上已实证**(2026-09-24 私仓合成信封): run 36009026539 top-1=reward ⇒ 零调用不发(负路径); run 36009569537 top-1=display 0.77 稳定 ⇒ 2 张证书 10.6 s 都 UPGRADED 共同见证 ⇒ ③′ CITED_UNVERIFIED, citable 恒 False, 产物无原文(正路径); 同 run 实证 reader∥s1 重叠(链 73 s vs 串行 ≥145 s)。③′ 仍非语义正确, ④ 未实现。 "
