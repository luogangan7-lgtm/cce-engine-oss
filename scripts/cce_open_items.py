...
def items() -> list[dict]:
    ...
    try:
        ...
        out.append({"类": OPEN,
                    ...
                    "证据": ("原问题: " + rv["⑥★★★新增的 owner 待决项"]["问题"] +
                            ...
                            " ★★★ **仍未做, 所以留在 OPEN 不是关闭**: "
                            "① 谓词**没有生产调用方** —— citable_as_confirmed() 至今 0 处被 cce_full_run / reply_loop 调用; "
                            "接进去要动生产输出路径, 且要先定**谁来提供证据片段**。 "
                            "② **证据片段目前无人产出**: 本层要求「原文里逐字可指的片段 + 它关于哪个对象 + 属于合同枚举的哪种增量」, "
                            "而产出这些要么靠模型(回到 prompt 面, 已测出有害)要么靠人 —— **这是个未决的设计选择, 不是我能单方面定的**。 "
        ...
        out.append({"类": OPEN,
                    "项": f"★ 断言表 v2 有 **{nw}** 条已撤销 —— 撤销项对应的用例回到**未裁定**, 需要新断言",
    ...
    if os.path.exists(_rbp):
        ...
        out.append({"类": BLOCKED,
                    "项": "五类成立条件**升不升合同**: 卡在「附件 A 里根本没有这五条定义文本」",
