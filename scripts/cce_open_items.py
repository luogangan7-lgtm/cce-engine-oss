    ...
    try:
        ...
        out.append({"类": OPEN,
                    ...
                    "证据": ("原问题: " + rv["⑥★★★新增的 owner 待决项"]["问题"] +
                            ...
                            "② **证据片段的产出方已定**(同日代定): = s2 模型的 evidence_quote(本来就在产出里, 此前从未核过是否逐字在原文); 但 s2 协议不给「支撑哪一支 + 关于哪个对象」⇒ 资格层**结构上只能给候选**, 升格要走 r2 式引用证书协议(另立预注册)。★ 引用证书协议进生产: 2026-09-24 预注册并**试点已跑**(88 次): 判决 **STOP** —— MALFORMED 率 24%>10%, 可判率 24/41(CP95 0.42–0.74), 升格 4/41, 代价中位 20s ⇒ 不接线, 先修采集协议(格式稳定性); 下一版执行器要存 MALFORMED 原因码。 "
