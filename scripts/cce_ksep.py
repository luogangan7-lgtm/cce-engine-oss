...
#  3. ~~等长内容效应只有长度驱动效应的 18.2%, T0-T2 约 82% 是长度~~
#     —— **本条已被两次实测推翻, 2026-08-18, 撤回。**
#     (a) 第二对等长 T=0.22389 = T0-T2(0.40611) 的 **55%** ⇒ 18.2% 是拿**最小**的那个
#         等长效应当上限, 而等长效应本身在 0.074-0.224 间变动。
#     (b) run 32147076464 腿B: BASE 与 BASE×5(**内容逐字相同**, 长度 293→1473)
#         判 **EQUIVALENT**(T=0.02056, 等价上界 0.04514 < min_effect), 结集完全相同
#         ⇒ **长度 per se 根本不驱动读数。**
#     正确说法: 长度本身无效应; T0-T2 的差异应归因于内容。
#     ⚠️ 腿B 的限度: 重复文本是最弱的一种"变长", 模型可能把重复段落当一段处理。
