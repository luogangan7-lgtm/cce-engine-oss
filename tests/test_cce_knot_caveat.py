...
CURRENT = [
    # ★★★ 2026-09-07 第三次改动: belong 的「死类」判定**已撤销**(换帖子后 7/81=8.6%,
    #   判别效度 RETAIN_as_core), 且补上重复性/外部效度的实测边界。
    #   ★ 每一次改动都不是「变好了所以摘掉标注」, 而是**把标注写得更准**。
    '结分类学 v1: G-K1 两项指标达标但**面板从未被有效认证**(v2 三态下五人全 UNRESOLVED); **belong 在评论上是单元错配非死类** · G-K3 原判已撤销(重言) · G-K2 不可判 · **外部效度未被证伪但远未确立** · **类空间塌陷是仪器性质** —— 引用须带「未验」',
] + PRE_REFACTOR[1:]
